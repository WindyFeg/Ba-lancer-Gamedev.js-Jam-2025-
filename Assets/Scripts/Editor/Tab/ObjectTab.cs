    ...
    public class ObjectTab
    {
        ...
        public ObjectTab()
        {
            ...
            if (!LoadObjectData())
            {
                ...
                _objects.Add(new ObjectData("Object 1"));
            }
        ...
        private void InitializePreviousStatValues()
        {
            if (_objects.Count > 0)
            {
                ...
                ObjectData obj = _objects[_selectedObjectIndex];
        ...
        public void Draw()
        {
            EditorGUILayout.LabelField("Object Design", EditorStyles.boldLabel);
            ...
            {
                SaveObjectData();
                EditorUtility.DisplayDialog("Save Complete", "Object data saved successfully!", "OK");
            }
            ...
            if (GUILayout.Button("Load Data", GUILayout.Height(25)))
            {
                ...
                {
                    InitializePreviousStatValues();
                    EditorUtility.DisplayDialog("Load Complete", "Object data loaded successfully!", "OK");
                }
                else
                {
                    EditorUtility.DisplayDialog("Load Failed", "No saved object data found or error loading data.",
                        "OK");
                }
            }
            ...
            for (int i = 0; i < _objects.Count; i++)
            {
                ...
                if (GUILayout.Button(_objects[i].Name, tabStyle, GUILayout.Height(30)))
                {
            ...
            {
                _objects.Add(new ObjectData($"Object {_objects.Count + 1}"));
                _selectedObjectIndex = _objects.Count - 1;
                InitializePreviousStatValues();
            }

            if (_objects.Count > 0 && GUILayout.Button("Remove object", GUILayout.Height(30)))
            {
                if (EditorUtility.DisplayDialog("Confirm Delete",
                        $"Are you sure you want to delete '{_objects[_selectedObjectIndex].Name}'?", "Yes", "No"))
                {
                    _objects.RemoveAt(_selectedObjectIndex);
                    _selectedObjectIndex = Mathf.Clamp(_selectedObjectIndex, 0, _objects.Count - 1);
                    InitializePreviousStatValues();
                }
            }
            ...
            if (_objects.Count > 0)
            {
                DrawObject(_objects[_selectedObjectIndex]);
            }
        ...
        private bool DrawIntStatBar(string label, ref int value, Color barColor, ref int minValue, ref int maxValue)
        {
            EditorGUILayout.BeginHorizontal();
